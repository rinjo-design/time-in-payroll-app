...
export async function requireEmployee(){const id=await getSession('employee');if(!id)throw new Error('Select an employee before continuing.');return id;}
export async function requireAdmin(){const id=await getSession('admin');if(!id)throw new Error('Administrator sign-in required.');return id;}
